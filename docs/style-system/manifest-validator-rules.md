...
- `src/lib/style-engine/validator.ts` implements pure V1 validation for top-level shape, identity, source metadata, intent, required constraints, required token groups/tokens, unsafe string patterns, executable string rejection, direct URL rejection, file/blob URL rejection, data URL rejection, VBScript URL rejection, CSS import/block/declaration-list rejection, environment/workspace persistence string rejection, CSS variable namespace guarding, dynamic Tailwind arbitrary value class guarding, dynamic z-index/protected behavior class string guarding, recipe behavior-key scanning, focus-capable recipe warnings, recommended recipe slot warnings, React Flow behavior-key scanning, React Flow visual adapter slot warnings, primary and secondary text contrast for parseable hex and opaque RGB colors, and deterministic display-safe reports.
- `src/lib/style-engine/validator.test.ts` covers safe manifest acceptance, non-object root rejection, missing required top-level field rejection, identity metadata rejection, source metadata shape rejection without payload echo, intent metadata rejection including empty `mood`/`material` arrays, required safety constraints rejection, protected behavior class entry rejection, required token group/token/value rejection, recipe and adapter container shape rejection, unsafe string redaction, executable string rejection without payload echo, direct URL rejection without private host echo, file/blob URL rejection without payload echo, data URL and VBScript URL rejection without payload echo, CSS import/block/declaration-list rejection without payload echo, environment and workspace persistence string rejection without payload echo, generic HTML tag rejection, unapproved CSS variable reference rejection, approved NEXUS and legacy bridge CSS variable references, dynamic Tailwind arbitrary value class rejection, dynamic z-index/protected behavior class string rejection, legacy CSS expression rejection, recipe behavior rejection, focus recipe warnings, recommended recipe slot warnings, unknown recipe semantic token reference rejection, React Flow behavior rejection, React Flow visual adapter slot warnings, benign unknown top-level rejection, workspace/backend top-level pollution rejection, external platform top-level pollution rejection, and required `recipes.commandPalette` group rejection.
- `src/lib/style-engine/accessibility.test.ts` covers WCAG-style contrast ratios for hex and opaque RGB/RGBA values, keeps translucent or unknown color formats as non-guessed `null` results, and confirms secondary text contrast rejection against an opaque RGB panel surface.
...
- Token value parsing now includes targeted unsafe string and CSS variable namespace guards plus accessibility parsing for hex and opaque RGB/RGBA colors, but is still not a full structured CSS/value parser.
- Accessibility validation currently covers parseable hex/opaque-RGB primary text contrast, parseable hex/opaque-RGB secondary text contrast against panel surfaces, and high-contrast intent warnings. It does not attempt translucent color compositing.
...
- `intent.mood` or `intent.material` is not a non-empty string array.
...
- `protectedBehaviorClasses` is not an array of non-empty strings.
...
- `url(`
- direct URL strings such as `http://`, `https://`, or `ftp://`
- local or opaque URL strings such as `file:` or `blob:`
