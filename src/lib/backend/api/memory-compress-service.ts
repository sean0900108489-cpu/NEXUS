...
import { createRequestValidator, validationIssue } from "./api-request-validator";
...
export type MemoryCompressRequest = {
  payload?: unknown;
...
export const validateMemoryCompressRequest = createRequestValidator<MemoryCompressRequest>(
  (value) => {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      return {
        ok: false,
        issues: [
          validationIssue([], "invalid_type", "Request body must be an object."),
        ],
      };
    }

    const record = value as MemoryCompressRequest;
    ...
    if (
      ...
      return {
        ok: false,
        issues: [
          validationIssue(["config"], "invalid_type", "config must be an object."),
        ],
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        issues: [
          validationIssue(["workspaceId"], "invalid_type", "workspaceId must be a string."),
        ],
      };
    }
...
export function sanitizeHeaderValue(value: string | null | undefined) {
...
export function normalizeConfig(value: unknown): IMemoryCompressionConfig {
  const record =
    value && typeof value === "object"
      ? (value as Partial<IMemoryCompressionConfig>)
      : {};
...
export async function executeMemoryCompression({
  apiKey,
  baseUrl,
  requestPayload,
}: {
  apiKey: string;
  baseUrl: string;
  requestPayload: MemoryCompressRequest;
}): Promise<ICompressedMemoryResult | { mockFallback: true; error?: string }> {
  if (!apiKey) {
    return { mockFallback: true };
  }

  const config = normalizeConfig(requestPayload.config);

  try {
    return await fetchCompressedMemory({
      apiKey,
      baseUrl,
      config,
    ...
    return { error: "compression-failed", mockFallback: true };
  }
}
...
async function fetchCompressedMemory({
  ...
  const profile =
    MEMORY_COMPRESSION_PROFILE_REGISTRY[
      config.compressorProfileId ?? DEFAULT_PROFILE_ID
