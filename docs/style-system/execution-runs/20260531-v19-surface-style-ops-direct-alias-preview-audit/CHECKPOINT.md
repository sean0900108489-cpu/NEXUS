...
Starting HEAD: `725c58a feat: expand surface style production alias bridge`

## Goal

Audit whether the Surface Style direct alias Bridge Plan coverage translates into
...
- Surface Style token preview did not change the Production Chrome Smoke specimen
  computed styles.
- The command palette smoke specimen stayed on the same dark/system baseline
  before and after Surface Style token preview:
  - background: `rgba(8, 16, 22, 0.78)`
  - border: `rgba(226, 232, 240, 0.12)`
  - radius: `4px`
  - blur: `8px`
- Separate Production Chrome Smoke apply/revert still changed/reverted styles,
  but those local smoke variables are not the Surface Style north-star palette.
