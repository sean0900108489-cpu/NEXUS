-- V22 workspace session RPC fallback.
--
-- Purpose:
-- - Preview deployments may intentionally lack SUPABASE_SERVICE_ROLE_KEY.
-- - New authenticated users still need a traceable owned workspace before
--   Workflow Pro / Runtime Lite can pass permission gates.
-- - This SECURITY DEFINER RPC uses auth.uid(); callers cannot provide or spoof
--   another user id.
...
COMMENT ON FUNCTION public.nexus_ensure_workspace_session(TEXT, TEXT) IS
  'V22 authenticated workspace session bootstrap. Uses auth.uid() only; no caller-supplied user id is accepted.';
