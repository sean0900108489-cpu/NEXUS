...
- Latest completed checkpoint: `CP-158 - Style Lab Static Prompt Vault Surface Specimen V1`.
- Latest local checkpoint commit: pending CP-158 local commit; previous clean checkpoint commit is `3ff6e14b1bad572bba420ba02df495043320428d`.
- Confirmed current branch during state assessment: `codex/v17-large-iteration`.
- Confirmed current status after CP-158 verification: dirty only in CP-158 allowed source/docs/run-doc files.
- Current stop reason: not stopped; CP-158 verification passed and local checkpoint commit prep is underway.

## In Progress

- `CP-158 - Local checkpoint commit prep`.
...
- Forbidden files: `src/components/nexus/**`, especially `src/components/nexus/PromptVaultManager.tsx`; `src/app/**`; runtime provider internals; `src/lib/style-engine/**`; CSS/global stylesheets; store/sync/backend/Supabase/database files; package/deploy files; AI/runtime API calls; React Flow imports or behavior props; Prompt Vault behavior semantics; command palette/window/modal behavior semantics; download/clipboard/save/export-file behavior; remote push; branch merge; deploy; database mutation; and `exports/**`.
- Verification: final `git diff --check`, `git status --porcelain=v1 -b`, and commit metadata check before selecting the next isolated unit.
- Rollback: revert only the isolated Style Lab Prompt Vault surface specimen, related phase-doc evidence lines, and this run-doc checkpoint update if the static Prompt Vault surface specimen must be removed.

## Next

1. Commit the CP-158 isolated Style Lab checkpoint if final diff/status checks remain clean by scope.
