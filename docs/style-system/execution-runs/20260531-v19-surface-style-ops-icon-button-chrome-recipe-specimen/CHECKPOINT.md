...
Starting HEAD: `5f1a10e docs: audit surface style polish gaps`

## Goal

Add a Style Lab-only Surface Style icon/button/badge/input-like chrome recipe
specimen.
...
- `surfaceStyleControlChromeIconButtons`
  - theme
  - alert
  - focus
  - new/add
...
- Style Lab can show a Surface Style control chrome recipe beside the existing
  scene, segmented nav, agent bank, workspace, right metrics, and chrome
  specimens.
- Existing local Surface Style preview variables can carry the glass surface,
...
- `https://cdn.example.com/nexus/bg-surface-shell.webp` failed to load with
...
Use the Surface Style control chrome recipe specimen as the visual standard, then
