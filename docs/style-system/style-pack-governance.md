...
- `src/lib/style-engine/governance.test.ts` and `src/lib/style-engine/exchange.test.ts` cover warning/validated/rejected states, compiler-failed manifest rejection without preview metadata, conservative permissions for draft/deprecated/retired/quarantined states, checksum shapes, validator version metadata, `adapterCoverage.reactFlow = complete`, `adapterCoverage.windowModal = complete`, preview variable counts, unknown import source rejection, unsupported package-like import source rejection, and unsafe import/export redaction.
