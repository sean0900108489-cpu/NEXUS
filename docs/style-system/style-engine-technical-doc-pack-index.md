...
| `style-pack-authoring-context-v1.md` | Copyable external LLM authoring context | Authoring owner | V2 contracts, authoring reference, Pixel Workshop fixture, Style Lab token preview source | Prompt template, minimal JSON, Pixel Workshop JSON, common rejection repairs, closed-loop usage | External models can generate reviewable Skin Pack JSON without widening validator authority. |
