...
- `src/lib/style-engine/exchange.ts` implements pure text-safe direct manifest and export-package import normalization plus redacted exchange review output, including validator version metadata while excluding the internal governance version.
- `src/lib/style-engine/governance.test.ts` and `src/lib/style-engine/exchange.test.ts` cover warning/validated/rejected states, compiler-failed manifest rejection without preview metadata, conservative permissions for draft/deprecated/rejected/retired/quarantined states, checksum shapes, validator version metadata, `adapterCoverage.reactFlow = complete`, `adapterCoverage.windowModal = complete`, preview variable counts, direct manifest import normalization, unknown import source rejection, unsupported package-like import source rejection, and unsafe import/export redaction.
