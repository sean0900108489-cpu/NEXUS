...
Current unit is Style Lab Static Window Specimen V1 local checkpoint prep. Source logic beyond the isolated specimen, production graph/app shell files, runtime provider internals, workspace store, sync, backend, Supabase, branch merge, push, `exports/**`, and `src/components/nexus/nexus-ops.tsx` remain closed.
