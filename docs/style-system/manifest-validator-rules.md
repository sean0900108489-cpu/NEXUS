...
- `src/lib/style-engine/validator.test.ts` covers safe manifest acceptance, unsafe string redaction, generic HTML tag rejection, unapproved CSS variable reference rejection, dynamic Tailwind arbitrary value class rejection, legacy CSS expression rejection, recipe behavior rejection, focus recipe warnings, recommended recipe slot warnings, unknown recipe semantic token reference rejection, React Flow behavior rejection, workspace/backend top-level pollution rejection, and required `recipes.commandPalette` group rejection.
