...
- `src/lib/style-engine/window-modal-recipe-adapter.ts` provides a pure visual adapter for window, modal, and command palette recipe slots, maps command palette slots from `recipes.commandPalette`, and emits deterministic `--nexus-recipe-*` variables.
