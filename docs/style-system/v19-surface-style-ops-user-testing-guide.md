# V19 Surface Style Ops User Testing Guide
...
Use this guide to evaluate the current Surface Style Ops skinning work before V19
...
- known `bg-surface-shell.webp` placeholder failures are treated as baseline if
  they appear
...
2. Find `Surface Style Ops Coverage`.

3. Click `Use Surface Style`.
...
6. Confirm the coverage panel shows:
   ...
   - unsupported Surface Style target gaps

7. Use token preview.

8. Revert token preview.

9. Find `Surface Style Scene Preview`.
...
- Surface Style Scene Preview should show the north-star mood and recipe direction
...
- Style Lab can locally preview Surface Style variables and smoke chrome aliases
...
Surface:
Observed:
Expected Surface Style behavior:
Score 0-5:
Gap:
...
- Surface Style fixture cannot be reviewed or accepted
- token preview cannot revert
- Production Chrome Smoke cannot revert
- console shows a new error from the Surface Style panels
