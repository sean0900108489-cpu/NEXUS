...
Starting HEAD: `015f704 docs: audit surface style direct alias preview`

## Goal

Add a Style Lab-only Surface Style scene/wash preview so the Surface Style fixture and
...
- `/style-lab`
- section: `Surface Style Scene Preview`
- local isolated preview target:
  - `data-testid="surface-style-scene-preview-target"`

Design:

- local custom property map using already adopted Surface Style production alias
  names
...
The preview uses the same adopted alias names that the Surface Style Bridge Plan
...
- `docs/style-system/execution-runs/20260531-v19-surface-style-ops-style-lab-scene-wash-preview/CHECKPOINT.md`
...
- `npm run typecheck`
- `npm run lint -- src/components/style-engine/nexus-style-lab.tsx src/components/style-engine/nexus-style-lab-surface-style-coverage.test.ts src/components/style-engine/nexus-style-lab-production-chrome-smoke.test.ts`
- `npm run build`
...
that uses direct Surface Style aliases. Keep it isolated from right-dock
